...
const live_banner = bannerData.live_accounts_banner?.promo_link

const isTradingIframeVisible = ref(false);
const selectedMt5IdForTrading = ref<number | null>(null);
...
function openWebTrader(mt5Id: number | undefined) {
  if (!mt5Id) {
    console.error('MT5 ID is missing.');
    return;
  }
  selectedMt5IdForTrading.value = mt5Id;
  isTradingIframeVisible.value = true;
}


const iframeUrl = computed(() => {
  if (!selectedMt5IdForTrading.value) return '';
  return `https://webtrading.winprofx.com/terminal?utm_source=www.winprofx.com&mode=connect&lang=en&theme-mode=1&theme=blueRed&login=${selectedMt5IdForTrading.value}`;
});
...
<template>
  ...
  <div class="grid grid-cols-12 gap-y-10 gap-x-6 mt-3">
    <div class="col-span-12">
      ...
      <div class="flex flex-col lg:items-center lg:flex-row gap-y-8">
        <div class="text-xl uppercase pl-5" style="color: #368fbe">{{ $t('livemt5account.liveMT5account') }}</div>

        <div class="flex items-center lg:ml-auto gap-3.5 pr-10">
          <Button class="common-btn-item cbi-fill pr-10"
                  @click="isShowAddNewAccountVisible=true">
            <Lucide icon="PenLine" class="stroke-[1.3] w-4 h-4 mr-2"/>
            {{ $t('livemt5account.createNewAccount') }}
          </Button>
        </div>
      </div>
              ...
              <div class="flex flex-col">
                <template v-if="!loading">
                  <div class="grid grid-cols-12 mt-3">
                    ...
                    <div class="col-span-12 sm:col-span-4 ml-3 lg:col-span-2 xl:col-span-4">
                      <div class="mt-3.5 ">
                        <div>
                          <template v-if="accountList.length > 0">
                            <div class="grid grid-cols-1 sm:grid-cols-1 gap-2 pb-6">
...
<!--                                        {{formatCurrency(account.balance)}}-->
                    ...
                    <div v-if="accountList.length > 0"
                         class="col-span-12 sm:col-span-10 lg:col-span-8 xl:col-span-8 flex items-center justify-center">

                      <div class="grid grid-cols-12 gap-y-10 gap-x-6">
                        <div class="flex flex-col col-span-12 xl:col-span-12 gap-y-2">
                          <div>
                            <div class="p-5 mt-3.5 box box--stacked bg-gradient-to-r from-[#2a1802] to-[#442702] text-white">
                                ...
                                <div class="flex flex-col gap-3 sm:items-center sm:flex-row">
                                  <div class="flex items-center">
                                    ...
                                    >
                                      ...
                                      >
                                        <img v-if="!accountDetails?.is_wallet" src="/mt5_logo.png" alt="icon"
                                             class="w-20 h-20"/>
                                        <img v-else src="/wallet.webp" alt="Paperbot" class="w-7 h-7">
                                      </div>
                                    </div>
                                    <div class="ml-4">
                                      <div class="text-xl text-slate-200 mt-0.5 text-[1.8rem] font-medium">
                                        {{ $t('livemt5account.mt') }}
                                        {{ accountDetails?.mt_id }}
                                      </div>
                                      <div class="text-slate-200 text-[1.2rem] mt-2">{{ $t('livemt5account.balance') }}
                                        : {{
                                  ...
                                  <div class="sm:ml-auto">
                                    <GradientButton @click="openWebTrader(accountDetails?.mt_id)" default-text="TRADE NOW" class="mb-3 w-1/2"/>
                                    <router-link to="/create-deposit" class="w-full">
                                      <Button
                                          variant="primary"
                                          class="common-btn-item cbi-fill w-full"
                                      >
                                        {{ $t('livemt5account.quickDeposit').toUpperCase() }}
                                      </Button>
                                    </router-link>
                                  </div>
                                </div>
                                    ...
                                    <div class="flex flex-col gap-x-6 sm:ml-auto">
                                      ...
                                      <div class="flex items-center gap-1">
                                          <span :class="[
                                            ...
                                            <span class="sm:ml-2 text-base"> 1:{{
                                      ...
                                      <div class="sm:ml-32">
                                        <a class="font-medium whitespace-nowrap text-xs text-slate-500 underline uppercase"
                                           @click="isUpdateLeverageDialogVisible=true">
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 w-full mt-1">

                          <!-- Update Password -->
                          <div class="flex-1 min-w-[190px] mr-3">
                            <Button class="w-full common-btn-item cbi-fill" @click="handleUpdatePassword">
                              <Lucide icon="SettingsIcon" class="stroke-[1.3] w-auto h-4 mr-2"/>
                          ...
                          <div class="flex-1 mr-3">
                            <router-link to="/create-deposit">
                              <Button class="w-full common-btn-item cbi-fill">
                                <Lucide icon="BetweenHorizontalStart" class="stroke-[1.3] w-4 h-4 mr-2"/>
                                {{ $t('livemt5account.deposit') }}
                              </Button>
                            </router-link>
                          </div>
                          ...
                          <div class="flex-1">
                            <router-link to="/create-withdraw">
                              <Button class="w-full common-btn-item cbi-fill">
                                <Lucide icon="BetweenHorizontalEnd" class="stroke-[1.3] w-4 h-4 mr-2"/>
                                {{ $t('livemt5account.withdraw') }}
                              </Button>
                            </router-link>
                          </div>
                        </div>
            ...
            <div>
              ...
              <img v-else :src="live_banner" class="rounded-lg">
            </div>
          </div>
        </div>
      </div>
  ...
  <UpdateLeverageDialog v-if="accountDetails" :open="isUpdateLeverageDialogVisible" :accountDetails="accountDetails"
                        @close="isUpdateLeverageDialogVisible = false"/>

  <CreateLiveAccountDialog :accountList="accountList" :open="isShowAddNewAccountVisible"
                           @close="isShowAddNewAccountVisible = false"/>

  <UpdatePasswordDialog v-if="accountDetails" :open="isPasswordUpdateDialogVisible" :accountDetails="accountDetails"
                        @close="isPasswordUpdateDialogVisible = false"/>

  <div v-if="isTradingIframeVisible" class="fixed inset-0 z-50 bg-black/80 flex items-center justify-center">
    <div class="relative w-full h-full md:ml-60 lg:ml-60">
      <Button
          class="absolute top-5 right-5 z-10 bg-slate-800 text-white mt-[81px] md:ml-60 lg:ml-60"
          @click="isTradingIframeVisible = false"
      >
        Close
      </Button>
      <iframe
          v-if="iframeUrl"
          :src="iframeUrl"
          class="w-full h-[90vh] md:ml-12 mt-20 lg:pr-16"
          frameborder="0"
          allowfullscreen
      ></iframe>
    </div>
